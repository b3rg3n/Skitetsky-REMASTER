            ...
            for caption, action, chosen in items:
                if action and caption:
                    button:
                        background None
                        action action
                        text caption font 'source/BloggerSans.ttf' size 23 hover_size 25 at ec_tr_choice
                        xalign 0.5
...
screen preferences():
    ...
    use game_menu(_("Крутилки"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "check"
                    label _("Размер пиздежа")
                    textbutton _("Сделать больше") action [SetField(persistent,'bazarbig', True)]
                    textbutton _("Стандартный") action [SetField(persistent,'bazarbig', False)]

                vbox:

                    if config.has_music:
                        label _("Громкость музона")
